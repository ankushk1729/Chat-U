        ...
        <div className = {`SideBar  ${ChatUser?"hidden":"block w-full"} md:w-35% lg:w-30% md:block flex flex-col px-4 lg:px-6 py-4 relative`}>
            ...
            <section className = 'SideBar-info h-28% bg-gray flex flex-col justify-center items-center rounded-xl'>
               <div className = 'relative'>
                    ...
                    {auth.user.photoURL && !downloadURL ?<img className = 'rounded-full h-70px w-70px mobile:h-30px mobile:w-30px  md:h-40px md:w-40px lg:h-50px lg:w-50px object-contain ' src = {auth.user.photoURL} alt = 'pfp'></img>:downloadURL?<img alt = "pfp" src = {downloadURL} className = 'rounded-full  sm:h-30px sm:w-30px  md:h-40px md:w-40px lg:h-50px lg:w-50px '></img>:<FaUserAlt className = 'mobile:h-30px mobile:w-30px  md:h-40px md:w-40px lg:h-50px lg:w-50px' color = '#e3e3e3' /> }

                    {auth.user.providerData[0].providerId!== 'google.com' && <div onClick = {()=>setShowModal(true)} className = 'cursor-pointer absolute -right-1 -bottom-0.5 rounded-full bg-white p-0 md:p-0.5'><MdEdit fill = '#20243c' size = {16} className = ''/></div>}
               </div>
